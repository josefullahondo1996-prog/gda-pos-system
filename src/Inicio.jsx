...
const Inicio = ({ perfilUsuario }) => {
  ...
  const [vistaMobile, setVistaMobile] = useState('inicio');
  ...
  if (cajasAbiertasConDatos.length === 0) {
    if (vistaMobile === 'vender') {
      const productosDemo = [
        { id: 1, nombre: 'Leche 1L', precio: 4500, stock: 12 },
        { id: 2, nombre: 'Pan de queso', precio: 3200, stock: 9 },
        { id: 3, nombre: 'Gaseosa 600ml', precio: 5500, stock: 18 },
        { id: 4, nombre: 'Yogur natural', precio: 3800, stock: 7 },
      ];
      const carritoDemo = [
        { id: 1, nombre: 'Leche 1L', cantidad: 2, precio: 4500 },
        { id: 3, nombre: 'Gaseosa 600ml', cantidad: 1, precio: 5500 },
      ];
      const totalDemo = carritoDemo.reduce((sum, item) => sum + item.precio * item.cantidad, 0);
        ...
        <div className="min-h-screen bg-[#efefed] text-slate-900">
          <div className="mx-auto min-h-screen max-w-[430px] bg-[#efefed] px-4 pb-0 pt-2">
            ...
            <header className="flex items-center justify-between px-1 pb-4 pt-1">
              <button className="flex h-10 w-10 items-center justify-center text-slate-800" onClick={() => setVistaMobile('inicio')}>
            ...
            <main className="space-y-4 px-1 pb-24 pt-1">
              ...
              <div className="grid grid-cols-2 gap-3">
                {productosDemo.map((producto) => (
                  <button
                    key={producto.id}
                    className="rounded-2xl bg-white p-3 text-left shadow-sm ring-1 ring-gray-200 transition hover:shadow-md"
                    onClick={() => alert(`Se agregó ${producto.nombre} al carrito`)}
                  >
                    <div className="mb-3 flex h-20 items-center justify-center rounded-xl bg-orange-50 text-orange-500">
                      <Package size={28} />
                    </div>
                    <p className="text-sm font-bold text-slate-800 line-clamp-2">{producto.nombre}</p>
                    <p className="mt-1 text-base font-black text-orange-500">Gs {producto.precio.toLocaleString('es-PY')}</p>
                    <p className="text-[11px] text-slate-500">Stock: {producto.stock}</p>
                  </button>
                ))}
              </div>
              ...
              <div className="rounded-2xl bg-white p-3 shadow-sm ring-1 ring-gray-200">
                <div className="mb-2 flex items-center justify-between">
                  <h3 className="text-base font-black text-slate-800">Carrito</h3>
                  <span className="text-xs font-bold text-slate-500">{carritoDemo.length} ítems</span>
                </div>
                <div className="space-y-2">
                  {carritoDemo.map((item) => (
                    <div key={item.id} className="flex items-center justify-between rounded-xl bg-slate-50 px-2 py-2 text-sm">
                      <div>
                        <p className="font-bold text-slate-800">{item.nombre}</p>
                        <p className="text-slate-500">{item.cantidad} x Gs {item.precio.toLocaleString('es-PY')}</p>
                      </div>
                      <span className="font-black text-slate-800">Gs {(item.precio * item.cantidad).toLocaleString('es-PY')}</span>
                    </div>
                  ))}
                </div>
                <div className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3">
                  <span className="text-sm text-slate-600">Total</span>
                  <span className="text-lg font-black text-orange-500">Gs {totalDemo.toLocaleString('es-PY')}</span>
                </div>
              </div>
            </main>

            <nav className="fixed inset-x-0 bottom-0 mx-auto max-w-[430px] border-t border-gray-200 bg-[#efefed] px-1 pb-5 pt-2">
              <div className="grid grid-cols-5 gap-2 text-center">
                {[
                  ...
                  <button
                    ...
                    onClick={() => label === 'Vender' && setVistaMobile('vender')}
                  >
        ...
        <div className="mx-auto min-h-screen max-w-[430px] bg-[#efefed] px-4 pb-0 pt-2">
          ...
          <main className="flex min-h-[calc(100vh-195px)] flex-col items-center justify-center px-2 pt-1 text-center">
            ...
            <div className="mt-9 w-full space-y-4">
              <button
                className="flex w-full items-center justify-center gap-3 rounded-[1.15rem] bg-orange-500 px-5 py-5 text-[2.05rem] font-black text-white shadow-[0_8px_18px_rgba(249,115,22,0.28)] transition-transform hover:translate-y-[-1px] active:scale-[0.99]"
                onClick={() => setVistaMobile('vender')}
              >
              ...
              <button
                className="flex w-full items-center justify-center gap-3 rounded-[1.15rem] border-[3px] border-orange-500 bg-transparent px-5 py-5 text-[1.8rem] font-black text-orange-500 transition-colors hover:bg-orange-50 active:bg-orange-100"
                onClick={() => setVistaMobile('vender')}
              >
            ...
            <div className="grid grid-cols-5 gap-2 text-center">
              {[
                ...
                <button
                  key={label}
                  className={`flex flex-col items-center justify-center gap-2 rounded-xl px-1 py-3 ${active ? 'bg-[#f0dfcd] text-slate-900 shadow-sm' : 'text-slate-700'}`}
                  onClick={() => setVistaMobile(label === 'Vender' ? 'vender' : 'inicio')}
                >
                  <Icon size={26} strokeWidth={2.2} />
                  <span className="text-[0.8rem] font-medium leading-none">{label}</span>
                </button>
